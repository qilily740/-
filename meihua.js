document.addEventListener('DOMContentLoaded', () => {
    ...
    let meihuaSubPage = null;
    let meihuaMainHtml = null;
    const meihuaModal = document.querySelector('.app-modal');
    const meihuaCloseBtn = meihuaModal ? meihuaModal.querySelector('.app-close-btn') : null;
    const meihuaTitle = meihuaModal ? meihuaModal.querySelector('.app-modal-title') : null;
    ...
    function renderAppEditPage() {
        ...
        if(!content) return;
        const mode = window.appData.iconMode || 'night';
        let html = `<div class="ios-group">
            <div class="ios-group-title">图标主题</div>
            <div class="set-row" id="iconModeToggle">
                <div class="set-row-label">${mode === 'day' ? '日间模式（白底黑字）' : '夜间模式（黑底白字）'}</div>
                <div class="set-row-arrow">${mode === 'day' ? '☀️' : '🌙'}</div>
            </div>
        </div>`;
        html += `<div class="ios-group"><div class="ios-group-title">点击图标更换图片；点击文字直接修改名称</div></div>`;
        ...
        html += `</div>`;
        html += `<div class="ios-group" style="margin-top:8px;"><div class="ios-cell app-edit-reset-cell">恢复默认</div></div>`;
        ...
        const resetCell = content.querySelector('.app-edit-reset-cell');
        if (resetCell) {
            resetCell.onclick = function () {
                showConfirm('确定要恢复所有应用图标和名称到默认吗？', function () {
                    window.appData.appList = DEFAULT_APP_LIST.map(item => ({ name: item.name, icon: item.icon }));
                    window.saveData();
                    window.renderAll();
                    renderAppEditPage();
                    showToast('已恢复默认');
        ...
        const modeToggle = content.querySelector('#iconModeToggle');
        if (modeToggle) {
            modeToggle.onclick = function () {
                if (typeof window.toggleIconMode === 'function') {
                    window.toggleIconMode();
                }
                renderAppEditPage();
            };
        }
    ...
    window.APP_LIST["9"] = {
        ...
        onMount: function () {
            meihuaSubPage = null;
            if (meihuaTitle) meihuaTitle.textContent = "美化设置";
            const modalContent = document.querySelector(".app-modal-content");
            if (!meihuaMainHtml) {
                meihuaMainHtml = modalContent.innerHTML;
            }
            modalContent.onclick = function (e) {
                ...
                switch (act) {
                    ...
                    case "openAppIconList":
                        meihuaSubPage = 'appEdit';
                        if (meihuaTitle) meihuaTitle.textContent = "修改应用名称与图标";
                        renderAppEditPage();
                        break;
                }
            }
    ...
    if (meihuaCloseBtn) {
        meihuaCloseBtn.onclick = function() {
            if (meihuaSubPage === 'appEdit') {
                meihuaSubPage = null;
                if (meihuaTitle) meihuaTitle.textContent = "美化设置";
                const modalContent = document.querySelector(".app-modal-content");
                if (modalContent && meihuaMainHtml) {
                    modalContent.innerHTML = meihuaMainHtml;
                }
            } else {
                window.closeApp();
            }
